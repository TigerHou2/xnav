%% Velocity Decomposition Test
%
% Author: Tiger Hou
%
% This script tests whether RROD can outperform VIOD simply by decomposing
% its velocity vector into x, y, and z components
...
%% Define Imaginary Pulsars (used for velocity decomposition)
...
numImPulsars = size(imPulsars,1); % counter the number of pulsars
...
RROD_Im_data = nan(numImPulsars,numObsvPerPulsar);
VIOD_data = nan(numGroups,3);
Tvect_RROD = (Mvect+Moffset) * sqrt(a^3/mu);
Tvect_Im_RROD = repmat(Tvect_RROD(1,:),numImPulsars,1);
...
for j = 1:numPulsars
    ...
    for k = 1:numObsvPerPulsar
        ...
        if j == 1
            ...
            for jj = 1:numImPulsars
                RROD_Im_data(jj,k) = imPulsarMat(:,jj)' * vn;
            end
...
[rRROD_im,vRROD_im] = rrod(RROD_Im_data,Tvect_Im_RROD,imPulsarMat,mu);
...
error_RROD_im = norm(rRROD_im-rTrue) / norm(rTrue) * 100;
...
errorData_RROD_Im(ii) = error_RROD_im;
...
subplot(4,1,2)
temp2 = errorData_RROD_Im * norm(rTrue)/100/1000;
temp2(toRemove) = [];
...
subplot(4,1,4)
temp4 = errorData_VIOD_timed * norm(rTrue)/100/1000;
temp4(toRemove) = [];
h4 = histogram(temp4,'FaceColor',corder(4,:));
h4.Normalization = 'probability';
legend('VIOD w/ time')

scale = 1;
width = min([h1.BinWidth,h2.BinWidth,h3.BinWidth,h4.BinWidth]);
h1.BinWidth = width * scale;
h2.BinWidth = width * scale;
h3.BinWidth = width * scale;
h4.BinWidth = width * scale;
...
limX = [0,max([xlim1(2),xlim2(2),xlim3(2),xlim4(2)])];
limY = [0,max([ylim1(2),ylim2(2),ylim3(2),ylim4(2)])] * 1.05;
