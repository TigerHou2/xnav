% Notes
% save plots weith specific conditions that give incorrect false solutions
% save plots!
%
% make slides and documentation. Also, comment on code!

% TEST CASE:
% How does IOD3V orbit predictions perform between small changes in true
% anomaly?
% Highly eccentric orbits? 
%   Change positions of measurements

% Conclusions
%
% This simulation assumes that measurements are taken in a fixed frame
%   that is only translated from the solar system basis (SSB). Sensor
%   noise is applied with limitations noted in the code.
% The reasoning is that any rotation can be adjusted using star trackers, 
%   while the issue of moving frames and non-inertial frames will be explored
%   in later cases.
%
% The search method used here is outdated w.r.t the assumptions made.
%   Specifically, searches in the second set can be performed along
%   the direction of r12, which we know is correct due to the absence
%   of rotation.
%
% As long as the velocity measurements used by IOD3V.m to find positions
%   have a smaller variance than the resolution of the position sets
%   created from XNAV measurements, a unique solution will be found and
%   will also be correct. 
% If the variance is greater than the resolution, it is possible to
%   generate false solutions, and this cannot be fixed by changing the
%   error tolerance. 
        ...
        if abs(norm(r12_cand)-norm(r12))<tol*norm(r12)
            ...
            % from the first two points
                ...
                if abs(norm(r23_cand)-norm(r23)) < ...
                   ...
                   abs(norm(cross(r12_cand,r23_cand)/norm(cross(r12_cand,r23_cand)) - ...
                        Kref)) < tol
                    % check for distance, angle, and plane orientation
