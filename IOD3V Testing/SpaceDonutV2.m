...
samples   = 030;            % number of points on the Fibonaccia sphere
...
hold on
% permutate noise combinations
idx = permn(1:samples,3);
idx1 = idx(:,1);
idx2 = idx(:,2);
idx3 = idx(:,3);
pointsCell(size(idx,1)) = {zeros(res,3)};
parfor i = 1:size(idx,1)
    pointsCell(i) = {zeros(res,3)};
end
params_sample = zeros(size(idx,1),6);
parfor j = 1:size(idx,1)
    m = idx1(j);
    n = idx2(j);
    p = idx3(j);
    VP = V + [N(m,:);N(n,:);N(p,:)];
...
%     pos_base = zeros(res,3);
%     pos_base(1:size(pos,1),1:size(pos,2)) = pos(:,1:3);
    pointsCell(j) = {pos(:,1:3)};
    if make_plot
%         plot3(pos(:,1), pos(:,2), pos(:,3),'Color','Blue');
        scatter3(pos(:,1), pos(:,2), pos(:,3),0.5,'b');
    end
    [a,e,i,omg,w,f] = Get_Orb_Params(r1,v1,mu);
    params_sample(j,:) = [a,norm(e),i,omg,w,f];
end
