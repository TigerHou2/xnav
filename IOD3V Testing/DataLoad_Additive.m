...
rng_val   = init_cond.rng;      % rng seed for sim
start_day = init_cond.day;      % measurements start x days after periapse
obsv_min  = init_cond.min;      % minimum number of measurement days
obsv_max  = init_cond.max;      % maximum number of measurement days
obsv_int  = init_cond.obi;      % observe every x days
trials    = init_cond.num;      % trials to run for each measurement bin
noise     = init_cond.rms;      % noise applied
mu        = init_cond.mu;       % gravitational parameter of orbiting body
r         = init_cond.pos;      % initial position vector
v         = init_cond.vel;      % initial velocity vector
notes     = init_cond.notes;    % notes on simulation setup
use_f     = init_cond.use_f;    % use true anomaly as bins instead of days
dur       = init_cond.dur;      % orbital period of true orbit
res       = init_cond.res;      % number of points to plot on IOD orbits
obsvPlot  = init_cond.interval; % observation count used for display
fName     = init_cond.fName;    % file name, which contains setup info
...
f = msgbox({['File Name: ' filename] ...
            ...
            ['Min. Observations: ' num2str(obsv_min)] ...
            ['Max. Observations: ' num2str(obsv_max)] ...
            ['Observation Interval: ' num2str(obsv_int) ' days'] ...
            ['Trials per Case: ' num2str(trials)] ...
            ''});
