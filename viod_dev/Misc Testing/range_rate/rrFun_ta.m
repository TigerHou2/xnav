function [v_diff,V] = rrFun_ta(C,obsv,mu,dt,testing)
...
P1 = obsv(:,1) * norm(obsv(:,1))^2 / norm(obsv(1:2,1))^2;
...
if delta < 0
    v_diff = ones(1,size(obsv,2)) * sqrt(-delta) * 10;
    V = nan;
    return
end
...
V  = nan(size(obsv,1),size(obsv,2),4);
Vp = nan(size(obsv,1),size(obsv,2),4);
...
for i = 1:size(obsv,2)
    ...
    Vp(:,i,1) = dot(V(:,i,1),obsv(:,i)) / norm(obsv(:,i))^2 * obsv(:,i);
end
...
for i = 1:size(obsv,2)
    ...
    Vp(:,i,2) = dot(V(:,i,2),obsv(:,i)) / norm(obsv(:,i))^2 * obsv(:,i);
end
...
for i = 1:size(obsv,2)
    ...
    Vp(:,i,3) = dot(V(:,i,3),obsv(:,i)) / norm(obsv(:,i))^2 * obsv(:,i);
end
...
for i = 1:size(obsv,2)
    ...
    Vp(:,i,4) = dot(V(:,i,4),obsv(:,i)) / norm(obsv(:,i))^2 * obsv(:,i);
end

% --- compare projections with observations
Vd = reshape(vecnorm(Vp-obsv,2,1),size(obsv,2),4)';
