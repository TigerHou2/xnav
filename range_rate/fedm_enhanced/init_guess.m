function initGuess = init_guess(...
    r_f0,r_e,r_dM,res,rangeRateData,pulsars,Tvect,trueVars,mu,plot)
%INIT_GUESS searches a given space for fmin at a fixed resolution.
...
for k = 1:res(3)
    ...
    out = objFcn(fin,rangeRateData,Tvect,pulsars,mu);
    dat(i,j,k) = norm(out(:));
    F(i,j,k) = r_f0(i);
    E(i,j,k) = r_e(j);
    M(i,j,k) = r_dM(k);
...
D = dat(:);
lb = quantile(D,0.04);
ub = quantile(D,0.06); % find the lowest 1% of values
...
% create refined mesh
adv_res = res * 5; % refine the mesh more in each dimension
...
initGuess = [adv_r_f0(i(1)),adv_r_e(j(1)),adv_r_dM(k(1))];
...
D = D(D>lb&D<ub);
hold on
scatter3(trueVars(1),trueVars(2),trueVars(3),24,'cyan',...
        'DisplayName','True Soln',...
        'LineWidth',1.5)
scatter3( initGuess(1), initGuess(2), initGuess(3),24,'green',...
        'DisplayName','Guess',...
        'LineWidth',1.5)
